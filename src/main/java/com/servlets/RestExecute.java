...
public class RestExecute {
    ...
    private String txtPath = "C:\\rest1\\src\\main\\webapp\\WEB-INF\\classes";
    ...
    public Response someMethod(String javaCode) {
        ...
        String nameFile=uniqueFileName();
    ...
    private String uniqueFileName(){
        File directory = new File(txtPath);
        String fileName="";
        for(boolean trueName=false;trueName!=true;){
            Random r = new Random();
            fileName = String.valueOf(Math.abs(r.nextInt()));
            trueName = true;
            for (File e : directory.listFiles()) {
                if(fileName.equals(e.getName())==true){
                    trueName = false;
